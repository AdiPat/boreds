...
exports.func = functions.https.onCall((data, context) => {
  ...
  if(taskTitle.length == 0) {
      throw new functions.https.HttpsError('invalid-argument', 'Task title is empty.');
  }

  if(taskDescription.length == 0) {
    throw new functions.https.HttpsError('invalid-argument', 'Task description is empty.');
  }

  if(taskTitle.length > CONSTANTS.MAX_TASK_TITLE_LENGTH) {
    throw new functions.https.HttpsError('invalid-argument', `Task title length should be less than ${CONSTANTS.MAX_TASK_TITLE_LENGTH}, found ${taskTitle.length}.`);
  }

  if(taskDescription.length > CONSTANTS.MAX_TASK_DESCRIPTION_LENGTH) {
    throw new functions.https.HttpsError('invalid-argument', `Task description length should be less than ${CONSTANTS.MAX_TASK_DESCRIPTION_LENGTH}, found ${taskDescription.length}.`);
  }
  ...
  await newTaskRef.set(taskData).then(() => {
      ...
      if(err) {
        ...
        newTaskRef.remove(); 
        throw new functions.https.HttpsError('internal', `Failed to create task | taskTitle=${title} | taskDescription=${description}`);
      }
  });
